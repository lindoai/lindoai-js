...
export class WorkflowsResource {
   ...
   * Returns a `workflow_id` immediately — poll {@link getWebsiteStatus} with that id
   * to track progress and get the final result.
   *
   * @param request - The website creation request
   * @returns `{ success, workflow_id }`. Pass `workflow_id` to {@link getWebsiteStatus}.
   *
   * @example
   * ```typescript
   * const { workflow_id } = await client.workflows.createWebsite({
   ...
   * // Poll until done
   * while (true) {
   *   const status = await client.workflows.getWebsiteStatus(workflow_id);
   *   if (status.done) {
   ...
   * @returns `{ success, workflow_id }`. Pass `workflow_id` to {@link getPageStatus}.
   *
   * @example
   * ```typescript
   * const { workflow_id } = await client.workflows.createPage('website-123', {
   ...
   *
   * @param workflowId - The `workflow_id` returned by {@link createPage}
   * @returns The page workflow status envelope
   *
   * @example
   * ```typescript
   * const status = await client.workflows.getPageStatus('wf_abc123');
   * if (status.done && status.result) {
   *   console.log(`Published at ${status.result.path}`);
   * }
   * ```
   */
  async getPageStatus(workflowId: string): Promise<PageWorkflowStatusResponse> {
    return this.http.get<PageWorkflowStatusResponse>(
      `/v1/ai/workspace/page/status/${workflowId}`
    );
  }
   ...
   *
   * @param workflowIds - Workflow IDs returned by {@link createPage} / {@link batchCreatePages}.
   */
