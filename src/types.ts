...
// ----------------------------------------------------------------------------
// Used by `client.workflows.createWebsite/editWebsite/createBlog` and the
// matching `getWebsiteStatus/getEditStatus/getBlogStatus` poll helpers.
// All three create/edit endpoints return the same shape; the status endpoints
// share an envelope but differ in the shape of `result`.
// ============================================================================

/**
 * Response returned by `createWebsite`, `editWebsite`, `createBlog`.
 *
 * Deliberately minimal: pass `result.workflow_id` to the matching status endpoint
 * to poll progress. Create-website also returns `result.website_id` immediately.
 */
export interface CreateWorkflowResponse {
  ...
  result: {
    /**
     * Unique identifier for this workflow. Pass it to
     * `getWebsiteStatus` / `getEditStatus` / `getBlogStatus` to poll.
     */
    workflow_id: string;
    /** Preallocated website ID returned by createWebsite while generation runs. */
    website_id?: string;
  };
}
...
export interface WorkflowStatusEnvelope<Result = never> {
  /** The status poll succeeded. Check `result.status` for the workflow's own outcome. */
  success: boolean;
...
/** A page changed by a website-edit workflow. */
export interface EditWebsitePageEntry {
  page_id?: string;
  /** Full URL path on the website, e.g. `"/about"`. */
  path: string;
  title?: string;
...
export interface EditWebsiteWorkflowResult {
  website_id: string;
  /** How many pages the edit published. */
  pages_changed: number;
  /** The pages the edit changed and published. */
  updated_pages: EditWebsitePageEntry[];
}
...
export type EditWebsiteStatusResponse = WorkflowStatusEnvelope<EditWebsiteWorkflowResult>;
...
/**
 * Per-workflow entry in a batch status response. Same shape as the singular
 * status envelope but the top-level `success` boolean also signals whether the
 * workflow was found (`false` for not-found / wrong-type workflows).
 */
...
/**
 * Credit balance breakdown shared by workspace and client credits.
 */
...
/**
 * Response from GET /v1/ai/credits — workspace-level credit balance.
 */
...
/**
 * Response from GET /v1/ai/credits/client?client_id=... — client-level credit balance.
 */
...
/**
 * Request body for POST /v1/ai/credits/client/allocate.
 */
export interface AllocateClientCreditsRequest {
  client_id: string;
  credit_type: 'monthly' | 'purchased' | 'daily';
  amount: number;
  source?: string;
  notes?: string;
}
 ...
 */
export interface AllocateClientCreditsResponse {
  success: boolean;
  result?: {
    allocation?: {
      id: string;
...
export interface WebsiteDetailsResponse {
  ...
  result: {
    ...
    /** Markdown description of the website's design style, tone, and visual guidelines */
